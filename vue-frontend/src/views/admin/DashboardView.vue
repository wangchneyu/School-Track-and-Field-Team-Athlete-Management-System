<template>
  <div>
    <!-- Welcome banner -->
    <div class="admin-hero animate-fade-in mb-6">
      <div class="admin-hero-bg" />
      <div class="admin-hero-content">
        <div>
          <h1 class="admin-hero-title">{{ greeting }}，管理员</h1>
          <p class="admin-hero-sub">系统运行正常 · {{ todayStr }}</p>
        </div>
        <div class="admin-hero-actions hidden sm:flex">
          <RouterLink to="/admin/notifications" class="hero-action-btn">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v12m6-6H6"/></svg>
            发布通知
          </RouterLink>
          <RouterLink to="/admin/training" class="hero-action-btn hero-action-secondary">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            安排训练
          </RouterLink>
        </div>
      </div>
    </div>

    <!-- Stats cards -->
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <div v-for="(stat, i) in statsCards" :key="stat.label" class="admin-stat-card animate-fade-in" :style="{ animationDelay: i * 80 + 'ms' }">
        <div class="admin-stat-icon" :style="{ background: stat.iconBg }">
          <span v-html="stat.icon" />
        </div>
        <div class="admin-stat-info">
          <span class="admin-stat-value">{{ stat.value }}</span>
          <span class="admin-stat-label">{{ stat.label }}</span>
        </div>
        <div class="admin-stat-trend" v-if="stat.trend">
          <span :class="stat.trendUp ? 'text-success' : 'text-muted-foreground'" class="text-[10px] font-semibold">{{ stat.trend }}</span>
        </div>
      </div>
    </div>

    <!-- Main content grid -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
      <!-- Featured event + quick actions -->
      <div class="lg:col-span-2 space-y-5">
        <!-- Photo carousel -->
        <div class="hero-carousel card-base overflow-hidden">
          <div class="hero-slides" :style="{ transform: `translateX(-${currentSlide * 100}%)` }">
            <div v-for="(img, i) in slides" :key="i" class="hero-slide">
              <img :src="img.src" :alt="img.alt" class="hero-img" />
              <div class="hero-caption">
                <span class="badge-accent">{{ img.tag }}</span>
                <h3 class="hero-caption-title">{{ img.title }}</h3>
              </div>
            </div>
          </div>
          <div class="hero-dots">
            <button v-for="(_, i) in slides" :key="i" :class="['hero-dot', { active: i === currentSlide }]" @click="currentSlide = i" />
          </div>
          <button class="hero-arrow hero-arrow-left" @click="prevSlide">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
          </button>
          <button class="hero-arrow hero-arrow-right" @click="nextSlide">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
          </button>
        </div>

        <!-- Featured event -->
        <div class="card-base p-5">
          <div class="section-header mb-4">
            <h3 class="text-sm font-bold text-foreground flex items-center gap-1.5">
              <svg class="w-4 h-4 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              近期重要赛事
            </h3>
            <RouterLink to="/admin/events" class="text-xs font-medium text-accent hover:underline">管理赛事 →</RouterLink>
          </div>
          <div v-if="featured" class="flex items-start gap-4">
            <div class="w-12 h-12 rounded-lg bg-accent/10 flex items-center justify-center flex-shrink-0">
              <svg class="w-6 h-6 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <div class="flex-1 min-w-0">
              <p class="font-semibold text-foreground">{{ featured.name }}</p>
              <p class="text-sm text-muted-foreground mt-0.5">{{ featured.location }}</p>
              <p class="text-xs text-muted-foreground mt-1">{{ featured.description }}</p>
              <div v-if="featured.countdown_days != null" class="mt-3 badge-accent">
                距离开赛还有 {{ Math.ceil(featured.countdown_days) }} 天
              </div>
            </div>
          </div>
          <p v-else class="text-sm text-muted-foreground">暂无近期赛事</p>
        </div>
      </div>

      <!-- Quick actions sidebar -->
      <div class="space-y-5">
        <div class="card-base p-5">
          <h3 class="text-sm font-bold text-foreground mb-4 flex items-center gap-1.5">
            <svg class="w-4 h-4 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
            快捷操作
          </h3>
          <div class="space-y-1.5">
            <RouterLink v-for="action in quickActions" :key="action.path" :to="action.path" class="quick-action-item">
              <div :class="['quick-action-icon', action.bg]">
                <span v-html="action.icon" />
              </div>
              <span class="text-sm font-medium text-foreground">{{ action.label }}</span>
              <svg class="w-4 h-4 text-muted-foreground ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            </RouterLink>
          </div>
        </div>

        <!-- Recent notifications -->
        <div class="card-base p-5">
          <div class="section-header mb-3">
            <h3 class="text-sm font-bold text-foreground flex items-center gap-1.5">
              <svg class="w-4 h-4 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
              最近通知
            </h3>
            <RouterLink to="/admin/notifications" class="text-xs font-medium text-accent hover:underline">管理 →</RouterLink>
          </div>
          <div v-if="recentNotifs.length" class="space-y-2">
            <div v-for="n in recentNotifs" :key="n.id" class="recent-notif-item">
              <div class="recent-notif-dot" :class="'type-' + n.type" />
              <div class="flex-1 min-w-0">
                <p class="text-xs font-medium text-foreground truncate">{{ n.title }}</p>
                <p class="text-[10px] text-muted-foreground">{{ n.read_count }}/{{ n.total_target }} 已读</p>
              </div>
            </div>
          </div>
          <p v-else class="text-xs text-muted-foreground">暂无通知</p>
        </div>
      </div>
    </div>

    <!-- Attendance stats -->
    <div class="card-base p-5 animate-fade-in" style="animation-delay: 400ms">
      <div class="section-header mb-4">
        <h3 class="text-sm font-bold text-foreground flex items-center gap-1.5">
          <svg class="w-4 h-4 text-success" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          出勤率概览
        </h3>
        <RouterLink to="/admin/attendance" class="text-xs font-medium text-accent hover:underline">详细数据 →</RouterLink>
      </div>
      <div v-if="attendanceStats.length" class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2.5">
        <div v-for="stat in attendanceStats.slice(0, 10)" :key="stat.athlete_id" class="flex items-center gap-3">
          <span class="text-xs font-medium text-foreground w-16 truncate">{{ stat.name }}</span>
          <div class="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
            <div class="h-full rounded-full transition-all duration-700" :style="{ width: stat.rate + '%', background: stat.rate >= 80 ? 'hsl(var(--success))' : stat.rate >= 60 ? 'hsl(var(--warning))' : 'hsl(var(--destructive))' }" />
          </div>
          <span class="text-[10px] font-semibold w-10 text-right" :style="{ color: stat.rate >= 80 ? 'hsl(var(--success))' : stat.rate >= 60 ? 'hsl(var(--warning))' : 'hsl(var(--destructive))' }">{{ stat.rate }}%</span>
        </div>
      </div>
      <p v-else class="text-sm text-muted-foreground">暂无出勤数据</p>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, computed, onMounted, onUnmounted } from 'vue'
import { RouterLink } from 'vue-router'
import { featuredEventApi, statsApi, athleteApi, sessionApi, notificationApi } from '@/api'
import type { FeaturedEvent, Notification } from '@/types'

const currentSlide = ref(0)
let slideTimer = 0

const greeting = computed(() => {
  const h = new Date().getHours()
  if (h < 6) return '夜深了'
  if (h < 9) return '早上好'
  if (h < 12) return '上午好'
  if (h < 14) return '中午好'
  if (h < 18) return '下午好'
  return '晚上好'
})

const todayStr = computed(() => {
  const d = new Date()
  const weekdays = ['日', '一', '二', '三', '四', '五', '六']
  return `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日 星期${weekdays[d.getDay()]}`
})

const slides = [
  { src: '/images/hero-banner.png', alt: '田径队训练', tag: '精彩瞬间', title: '日常训练风采' },
  { src: '/images/activity-1.png', alt: '跳高训练', tag: '专项训练', title: '跳高技术提升训练' },
  { src: '/images/activity-2.png', alt: '接力赛', tag: '团队协作', title: '接力队配合训练' },
  { src: '/images/activity-3.png', alt: '团队合影', tag: '团队风采', title: '赛后团队合影' },
]

function nextSlide() { currentSlide.value = (currentSlide.value + 1) % slides.length }
function prevSlide() { currentSlide.value = (currentSlide.value - 1 + slides.length) % slides.length }

const athleteCount = ref(0)
const sessionCount = ref(0)
const featured = ref<FeaturedEvent | null>(null)
const attendanceStats = ref<Array<{ athlete_id: number; name: string; rate: number }>>([])
const recentNotifs = ref<Notification[]>([])

const statsCards = ref([
  { label: '运动员', value: '—', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>', iconBg: 'hsl(var(--primary) / 0.12)', trend: '', trendUp: true },
  { label: '训练课次', value: '—', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>', iconBg: 'hsl(var(--accent) / 0.12)', trend: '', trendUp: true },
  { label: '平均出勤率', value: '—', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>', iconBg: 'hsl(var(--success) / 0.12)', trend: '', trendUp: true },
  { label: '距比赛', value: '—', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>', iconBg: 'hsl(var(--warning) / 0.12)', trend: '', trendUp: false },
])

const quickActions = [
  { path: '/admin/athletes', label: '添加运动员', bg: 'bg-primary', icon: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>' },
  { path: '/admin/training', label: '安排训练', bg: 'bg-accent', icon: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>' },
  { path: '/admin/scores', label: '录入成绩', bg: 'bg-success', icon: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6m6 0V9a2 2 0 012-2h2a2 2 0 012 2v10"/></svg>' },
  { path: '/admin/notifications', label: '发布通知', bg: 'bg-warning', icon: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>' },
]

async function loadDashboard() {
  try {
    const [athleteRes, sessionRes, featuredRes, statsRes, notifRes] = await Promise.allSettled([
      athleteApi.list(),
      sessionApi.list(),
      featuredEventApi.get(),
      statsApi.attendance(),
      notificationApi.list({ limit: '5' }),
    ])

    if (athleteRes.status === 'fulfilled') {
      const list = athleteRes.value.data
      athleteCount.value = Array.isArray(list) ? list.length : 0
      statsCards.value[0].value = String(athleteCount.value)
    }
    if (sessionRes.status === 'fulfilled') {
      const list = sessionRes.value.data
      sessionCount.value = Array.isArray(list) ? list.length : 0
      statsCards.value[1].value = String(sessionCount.value)
    }
    if (featuredRes.status === 'fulfilled' && featuredRes.value.data) {
      featured.value = featuredRes.value.data
      if (featured.value?.countdown_days != null) {
        statsCards.value[3].value = Math.ceil(featured.value.countdown_days) + '天'
      }
    }
    if (statsRes.status === 'fulfilled') {
      const data = statsRes.value.data
      if (Array.isArray(data)) {
        attendanceStats.value = data
        const avg = data.length > 0 ? Math.round(data.reduce((s: number, a: { rate: number }) => s + a.rate, 0) / data.length) : 0
        statsCards.value[2].value = avg + '%'
      }
    }
    if (notifRes.status === 'fulfilled') {
      recentNotifs.value = notifRes.value.data || []
    }
  } catch {
    // dashboard load errors are non-critical
  }
}

onMounted(() => {
  loadDashboard()
  slideTimer = window.setInterval(nextSlide, 5000)
})

onUnmounted(() => {
  clearInterval(slideTimer)
})
</script>

<style scoped>
/* Admin hero */
.admin-hero {
  position: relative;
  border-radius: var(--radius-lg);
  overflow: hidden;
  padding: 1.5rem;
}
.admin-hero-bg {
  position: absolute;
  inset: 0;
  background: var(--gradient-hero);
}
.admin-hero-content {
  position: relative;
  z-index: 1;
  display: flex;
  align-items: center;
  justify-content: space-between;
}
.admin-hero-title {
  font-size: 1.25rem;
  font-weight: 700;
  color: hsl(210 40% 98%);
}
.admin-hero-sub {
  font-size: 0.75rem;
  color: hsl(210 30% 65%);
  margin-top: 0.25rem;
}
.admin-hero-actions {
  gap: 0.625rem;
}
.hero-action-btn {
  display: inline-flex;
  align-items: center;
  gap: 0.375rem;
  padding: 0.5rem 1rem;
  border-radius: var(--radius-md);
  font-size: 0.8125rem;
  font-weight: 600;
  background: hsl(var(--accent));
  color: hsl(var(--accent-foreground));
  transition: var(--transition-smooth);
}
.hero-action-btn:hover {
  transform: translateY(-1px);
  box-shadow: 0 4px 12px hsl(var(--accent) / 0.3);
}
.hero-action-secondary {
  background: hsl(0 0% 100% / 0.1);
  color: hsl(210 40% 98%);
}
.hero-action-secondary:hover {
  background: hsl(0 0% 100% / 0.15);
  box-shadow: none;
}

/* Stat cards */
.admin-stat-card {
  display: flex;
  align-items: center;
  gap: 0.75rem;
  padding: 1rem;
  border-radius: var(--radius-md);
  background: hsl(var(--card));
  border: 1px solid hsl(var(--border));
  box-shadow: var(--shadow-card);
  transition: var(--transition-smooth);
}
.admin-stat-card:hover {
  box-shadow: var(--shadow-card-hover);
  transform: translateY(-1px);
}
.admin-stat-icon {
  width: 42px;
  height: 42px;
  border-radius: var(--radius-md);
  display: flex;
  align-items: center;
  justify-content: center;
  flex-shrink: 0;
}
.admin-stat-info {
  display: flex;
  flex-direction: column;
  flex: 1;
}
.admin-stat-value {
  font-size: 1.375rem;
  font-weight: 700;
  color: hsl(var(--foreground));
  line-height: 1.2;
}
.admin-stat-label {
  font-size: 0.6875rem;
  color: hsl(var(--muted-foreground));
  margin-top: 0.125rem;
}

/* Quick action items */
.quick-action-item {
  display: flex;
  align-items: center;
  gap: 0.75rem;
  padding: 0.625rem 0.5rem;
  border-radius: var(--radius-sm);
  transition: var(--transition-fast);
}
.quick-action-item:hover {
  background: hsl(var(--muted) / 0.5);
}
.quick-action-icon {
  width: 32px;
  height: 32px;
  border-radius: var(--radius-sm);
  display: flex;
  align-items: center;
  justify-content: center;
  color: hsl(var(--accent-foreground));
  flex-shrink: 0;
}

/* Recent notification items */
.recent-notif-item {
  display: flex;
  align-items: center;
  gap: 0.625rem;
  padding: 0.375rem 0;
}
.recent-notif-dot {
  width: 8px;
  height: 8px;
  border-radius: 50%;
  flex-shrink: 0;
}
.recent-notif-dot.type-training { background: hsl(var(--accent)); }
.recent-notif-dot.type-announcement { background: hsl(var(--primary-light)); }
.recent-notif-dot.type-general { background: hsl(var(--muted-foreground)); }

/* Carousel (reused styles) */
.hero-carousel {
  position: relative;
  height: 200px;
  border-radius: var(--radius-lg);
}
@media (min-width: 768px) {
  .hero-carousel { height: 240px; }
}
.hero-slides {
  display: flex;
  height: 100%;
  transition: transform 0.5s cubic-bezier(0.4, 0, 0.2, 1);
}
.hero-slide {
  min-width: 100%;
  position: relative;
}
.hero-img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
.hero-caption {
  position: absolute;
  bottom: 0;
  left: 0;
  right: 0;
  padding: 1.25rem;
  background: linear-gradient(0deg, hsl(220 65% 10% / 0.7) 0%, transparent 100%);
}
.hero-caption-title {
  font-size: 1rem;
  font-weight: 700;
  color: hsl(210 40% 98%);
  margin-top: 0.375rem;
}
.hero-dots {
  position: absolute;
  bottom: 0.75rem;
  right: 1rem;
  display: flex;
  gap: 0.375rem;
}
.hero-dot {
  width: 6px;
  height: 6px;
  border-radius: 50%;
  background: hsl(0 0% 100% / 0.4);
  transition: var(--transition-fast);
}
.hero-dot.active {
  width: 18px;
  border-radius: 3px;
  background: hsl(var(--accent));
}
.hero-arrow {
  position: absolute;
  top: 50%;
  transform: translateY(-50%);
  width: 32px;
  height: 32px;
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  background: hsl(0 0% 100% / 0.2);
  backdrop-filter: blur(4px);
  color: hsl(210 40% 98%);
  transition: var(--transition-fast);
  opacity: 0;
}
.hero-carousel:hover .hero-arrow { opacity: 1; }
.hero-arrow:hover { background: hsl(0 0% 100% / 0.35); }
.hero-arrow-left { left: 0.75rem; }
.hero-arrow-right { right: 0.75rem; }
</style>
